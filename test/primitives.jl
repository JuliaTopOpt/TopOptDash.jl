@testset "Primitives" begin
  ...
  @testset "Bezier curves" begin
    ...
    BezierCurve = TopOptDash.BezierCurve
  ...
  @testset "Boxes" begin
    ...
    b = Box(P2(1,1), P2(2,2))
    @test sides(b) == T[1,1]
    @test TopOptDash.center(b) == P2(1.5,1.5)
    @test diagonal(b) == √T(2)
  ...
  @testset "Ball" begin
    ...
    @test paramdim(b) == 3
    @test coordtype(b) == T
    @test TopOptDash.center(b) == P3(1,2,3)
    @test radius(b) == T(5)
    @test isconvex(b)
  ...
  @testset "Sphere" begin
    ...
    @test paramdim(s) == 2
    @test coordtype(s) == T
    @test TopOptDash.center(s) == P3(0, 0, 0)
    @test radius(s) == T(1)
    @test !isconvex(s)
    ...
    @test TopOptDash.center(s) == P2(0.25, 0.75)
    @test radius(s) == T(0.7905694150420949)
    s = Sphere(P2(0,0), P2(1,0), P2(0,1))
    @test TopOptDash.center(s) == P2(0.5, 0.5)
